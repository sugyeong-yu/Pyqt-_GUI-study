...
form_class = uic.loadUiType('ui/train_tab.ui')[0] # ui파일 불러오기
class TabTrain(QWidget, form_class):
    def __init__(self, status_bar):
        super().__init__()
        self.setupUi(self)
        self.status_bar = status_bar # status bar를 받아와 변수로 저장

        # Initialize
        self.set_tree_view()
        self.set_widgets()
        self.set_events() # event달아주는 함수.
    ...
    def on_click_start(self):
        ...
        if data_path and batch_size and epochs and lr:
            # 학습을 이벤트에 넣으면 학습이 끝날때까지 이벤트가 끝날수 없음 이를 방지하기 위해서 학습을 별도의 흐름을 만들어줌.
            # 학습ㅇ르 한는 스레드는 q스레드를 상속받은 스레드르라ㄸ로만듬.
            ...
            # Show error message
            # 셋중에 문자열이 하나라도 입력이안되면ㅇ 에러메세지를 추력함.
            error_title = 'Error'
...
class TrainThread(QThread):
    # 큐스레드를 상속받음, 얘 자체가 시그널들을 가지고있음
    # 학습시작, 상태변화, 배치사이즈와에폭이 끝날때 발생하는 각각의 시그널 , 완전히 학습이 끝났을때 발생하는 시그널
    ...
    def run(self):
        # 학습에 대한 전반적인 내용. 1. 데이터로드 2. 모델생성 3. 반복문으로 setting하며 training 4. 모델저장
        # 스레드 스타트 > run이 실행됨.
    ...
    def set_model(self):
        self.model = CNN() # 밑에 Class 로 만들어놓음
        if torch.cuda.is_available():
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            for i, (images, labels) in enumerate(self.train_loader):
                ...
                self.end_batch_signal.emit(losses) # 배치가 끝났음을 알리는 시그널
            self.end_epoch_signal.emit() # 에폭이 끝났음을 알리는 시그널
        self.end_signal.emit(self.model) # 학습이 끝ㅌ났음을 알리는 시그널널
